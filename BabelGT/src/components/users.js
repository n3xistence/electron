...
const updateStepsContainer = (data) => {
  ...
  let stepDiffWeek1 =
    data.week1.steps[data.week1.steps.length - 1] - data.week1.steps[0];
  let stepDiffWeek2 =
    data.week2.steps[data.week2.steps.length - 1] - data.week2.steps[0];

  let stepRatio =
    stepDiffWeek1 > stepDiffWeek2
      ? (((stepDiffWeek2 - stepDiffWeek1) / stepDiffWeek1) * 100).toFixed(2)
      : Math.abs(
          ((stepDiffWeek2 - stepDiffWeek1) / stepDiffWeek1) * 100
        ).toFixed(2);
...
const updateLevelsContainer = (data) => {
  ...
  let levelDiffWeek1 =
    data.week1.levels[data.week1.levels.length - 1] - data.week1.levels[0];
  let levelDiffWeek2 =
    data.week2.levels[data.week2.levels.length - 1] - data.week2.levels[0];

  let levelRatio =
    levelDiffWeek1 > levelDiffWeek2
      ? (((levelDiffWeek2 - levelDiffWeek1) / levelDiffWeek1) * 100).toFixed(2)
      : Math.abs(
          ((levelDiffWeek2 - levelDiffWeek1) / levelDiffWeek1) * 100
        ).toFixed(2);
...
const updateNPCContainer = (data) => {
  ...
  let npcDiffWeek1 =
    data.week1.npc[data.week1.npc.length - 1] - data.week1.npc[0];
  let npcDiffWeek2 =
    data.week2.npc[data.week2.npc.length - 1] - data.week2.npc[0];

  let npcRatio =
    npcDiffWeek1 > npcDiffWeek2
      ? (((npcDiffWeek2 - npcDiffWeek1) / npcDiffWeek1) * 100).toFixed(2)
      : Math.abs(((npcDiffWeek2 - npcDiffWeek1) / npcDiffWeek1) * 100).toFixed(
          2
        );
...
const updatePVPContainer = (data) => {
  ...
  let pvpDiffWeek1 =
    data.week1.pvp[data.week1.pvp.length - 1] - data.week1.pvp[0];
  let pvpDiffWeek2 =
    data.week2.pvp[data.week2.pvp.length - 1] - data.week2.pvp[0];

  let pvpRatio =
    pvpDiffWeek1 > pvpDiffWeek2
      ? (((pvpDiffWeek2 - pvpDiffWeek1) / pvpDiffWeek1) * 100).toFixed(2)
      : Math.abs(((pvpDiffWeek2 - pvpDiffWeek1) / pvpDiffWeek1) * 100).toFixed(
          2
        );
...
const updateTimeframeDisplay = (week1, week2) => {
  ...
  let week1Timeframe = `${helper.dateToShortText(
    week1[0]
  )} - ${helper.dateToShortText(week1[week1.length - 1])}`;

  let week2Timeframe = `${helper.dateToShortText(
    week2[0]
  )} - ${helper.dateToShortText(week2[week2.length - 1])}`;
...
const updateUserNameContainer = (name) => {
  ...
  div.innerHTML = `<div class="inline-block">User:</div>
